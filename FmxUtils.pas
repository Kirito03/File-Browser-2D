...
begin
  Destination := ExpandFileName(DestName); { *Converted from ExpandFileName*  } { expand the destination path }
  if FileGetAttr(Destination)=16 then { if destination is a directory... }
  begin
...
begin
  ...
  begin
    if FileGetAttr(FileName)=1 then  { if it's read-only... }
...
begin
  try
    if FindFirst(ExpandFileName(FileName),faAnyFile,SearchRec) { *Converted from FindFirst*  } >= 0 then
      Result := SearchRec.Size
    else Result := -1;
  finally
    SysUtils.FindClose(SearchRec); { *Converted from FindClose*  }
  end;
end;
